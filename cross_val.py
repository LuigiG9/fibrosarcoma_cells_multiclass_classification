...
def load_data_kfold(k):
    ...
    folds = list(StratifiedKFold(n_splits=k, shuffle=True, random_state=1).split(x_train, y_train.argmax(1)))
    
    
    return  folds,x_train, y_train,x_test,y_test
k = 10
folds,X_train, y_train,X_test,y_test= load_data_kfold(k)
...
for j, (train_idx, val_idx) in enumerate(folds):
    ...
    train_generator=train_datagen.flow(X_train_cv, y_train_cv,
    ...
    my_file = 'graph.png'
    ...
    fig.savefig(os.path.join(my_path, str(j) +'.png'))   
    
    #Valutazione del modello sul test set locale e salvataggio risultati
    
    test_generator=valid_datagen.flow(X_test,y_test,batch_size=32)
